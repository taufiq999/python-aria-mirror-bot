...
def countNode(update, context):
    ...
    if len(args) > 1:
        link = args[1]
        msg = send_message(f"Counting: <code>{link}</code>", context.bot, update)
        ...
        if uname is not None:
            cc = f"\n\ncc: {uname}"
        send_message(result + cc, context.bot, update)
    else:
        send_message("Provide G-Drive Shareable Link to Count.", context.bot, update)

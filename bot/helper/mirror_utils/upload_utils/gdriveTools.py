...
class GoogleDriveHelper:
    ...
    def __upload_empty_file(self, path, file_name, mime_type, parent_id=None):
        ...
        file_metadata = {
            "name": file_name,
            "description": "Uploaded using Bot",
            "mimeType": mime_type,
        }
        if parent_id:
        ...
        return (
            ...
            .create(supportsAllDrives=True, body=file_metadata, media_body=media_body)
            .execute()
        )
    ...
    def deletefile(self, link: str):
        ...
        try:
            res = (
                self.__service.files()
                .delete(fileId=file_id, supportsAllDrives=IS_TEAM_DRIVE)
                .execute()
            )
    ...
    def __set_permission(self, drive_id):
        ...
        return (
            ...
            .create(supportsAllDrives=True, fileId=drive_id, body=permissions)
            .execute()
        )
    ...
    def upload_file(self, file_path, file_name, mime_type, parent_id):
        ...
        if os.path.getsize(file_path) == 0:
            ...
            response = (
                self.__service.files()
                .create(
                    supportsAllDrives=True, body=file_metadata, media_body=media_body
                )
                .execute()
            )
            ...
            drive_file = (
                self.__service.files()
                .get(supportsAllDrives=True, fileId=response["id"])
                .execute()
            )
    ...
    def clone(self, link):
        ...
        try:
            ...
            if meta.get("mimeType") == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                if BUTTON_FOUR_NAME and BUTTON_FOUR_URL:
                    buttons.buildbutton(f"{BUTTON_FOUR_NAME}", f"{BUTTON_FOUR_URL}")
                if BUTTON_FIVE_NAME and BUTTON_FIVE_URL:
                    buttons.buildbutton(f"{BUTTON_FIVE_NAME}", f"{BUTTON_FIVE_URL}")
                if BUTTON_SIX_NAME and BUTTON_SIX_URL:
        ...
        if response["files"]:
            ...
            for file in response.get("files", []):
                if (
                    ...
                    msg += f"<br><b>MD5:</b> <code>{file.get('md5Checksum')}</code>"
                msg += "<br><br>"
                content_count += 1
